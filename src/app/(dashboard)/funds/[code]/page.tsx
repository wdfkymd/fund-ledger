...
export default function FundDetailPage() {
  ...
  const chartData = useMemo(() => {
    ...
    // 只在约 5 个关键点显示 X 轴标签，避免 90 日刻度叠在一起
  ...
  if (error || !data) {
    return (
      <div className="mx-auto w-full max-w-xl px-5 py-16 text-center">
        <p className="text-sm text-muted-foreground">{error || "未找到基金"}</p>
        <Link
          href="/dashboard"
      ...
      <section className="mb-8 text-center">
        ...
        {holding && (
          <div className="mt-8 grid grid-cols-3 items-start divide-x rounded-xl border bg-muted/40 py-3.5 sm:py-4">
            <MetricCell
              label="持仓市值"
              value={fmt(holding.estimateValue)}
            />
            <MetricCell
              label="累计盈亏"
              value={signedMoney(holding.estimateProfit)}
              valueClassName={tone(holding.estimateProfit)}
            />
            <MetricCell
              label="今日"
              value={
